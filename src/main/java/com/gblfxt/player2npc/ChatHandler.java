...
import com.gblfxt.player2npc.compat.FTBTeamsIntegration;
...
public class ChatHandler {
    ...
    public static void onServerChat(ServerChatEvent event) {
        ...
        Player2NPC.LOGGER.debug("Chat message with prefix from {}: {}", player.getName().getString(), afterPrefix);
        ...
        if (companions.isEmpty()) {
            Player2NPC.LOGGER.debug("No companions found for player {} with target name '{}'",
        ...
        for (CompanionEntity companion : companions) {
            ...
            if (!isOwner && FTBTeamsIntegration.isModLoaded()) {
                ...
                if (owner != null) {
                    ...
                    if (isTeammate) {
                        Player2NPC.LOGGER.info("[{}] {} is a teammate of owner - granting command access",
                                companion.getCompanionName(), player.getName().getString());
                    }
                }
            }

            Player2NPC.LOGGER.info("[{}] Received message from {} (owner={}, teammate={}): {}",
