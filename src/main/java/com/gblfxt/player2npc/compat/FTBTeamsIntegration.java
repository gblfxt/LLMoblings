...
public class FTBTeamsIntegration {
    ...
    public static boolean isModLoaded() {
        if (!checkedLoaded) {
            ...
            checkedLoaded = true;
            if (isLoaded) {
                Player2NPC.LOGGER.info("FTB Teams detected - team member interaction enabled");
                initReflection();
            }
        }
        return isLoaded;
    ...
    private static void initReflection() {
        try {
            ...
            Player2NPC.LOGGER.info("FTB Teams reflection initialized successfully");
        } catch (Exception e) {
            Player2NPC.LOGGER.warn("Failed to initialize FTB Teams reflection: {}", e.getMessage());
            isLoaded = false;
        }
    }
    ...
    public static boolean areOnSameTeam(UUID uuid1, UUID uuid2) {
        ...
        try {
            ...
            Player2NPC.LOGGER.debug("Error checking FTB Teams membership: {}", e.getMessage());
        }
    ...
    public static String getTeamName(Player player) {
        ...
        try {
            ...
            Player2NPC.LOGGER.debug("Error getting FTB Team name: {}", e.getMessage());
        }
